...
class Game{
  ...
  Game(){
    ...
    boardState=List<int>(width).map(
     (_){
       List<int>.filled(height, 0);
     }
    ).toList();
  }
  ...
  void clearRows(){
    for(int idx=0;idx<rowState.length;idx++){
      ...
      if(row == width){
        ImageData imgd=ctx.getImageData(0, 0, cellSize*width,cellSize*idx);
        ctx.putImageData(imgd, 0, cellSize);
  ...
  bool pieceIMoving=true;

    ctx.fillStyle='gray';

    currentBlock.tiles.forEach(
      (Tile tile){
  ...
  void start(){
    ...
    Element ep=querySelector('#output');

    ep.nodes.add(board);
    ep.nodes.add(gameScore);
